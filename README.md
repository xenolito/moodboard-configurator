...
- **Dos modos de ejecución** — la app detecta automáticamente el contexto mediante `window.pdMoodboardConfig?.baseUrl` (inyectado por el plugin WordPress):
  - **Standalone** (desarrollo / embed directo): se muestra el `nav.ambient-switcher` para cambiar de ambiente
  - **WordPress** (`build:wp`): el switcher no se monta en el DOM; la navegación entre ambientes la gestiona WordPress a través del archive del CPT y el botón «Volver»
- **Botón volver** — muestra un botón `←` en la esquina inferior izquierda del viewer; la URL de destino se configura con `data-back-url` en el elemento raíz (`#ambient_viewer`), con `back_url` en el shortcode de WordPress, o con `backUrl` en el objeto ambient de `config.json`; si no se configura por ninguna vía, el fallback es `../` (directorio superior de la URL actual)
- **Panel de producto** — en móvil: siempre visible debajo del viewer con altura igual al espacio restante de la ventana (`100dvh - altura del viewer`); el viewer usa `position: sticky` para permanecer fijo mientras el usuario hace scroll por el panel; al llegar al final del panel el scroll continúa por el resto de la página; el botón toggle no se muestra en móvil. En desktop: panel lateral con altura igual al viewer; posición izquierda/derecha configurable (`panelSelectorPosition`)
...
- **Slider antes/después con compare independiente** — al activar el compare el panel se oculta; cada mitad (izquierda/derecha) tiene un estado de modelo y color propio; clicar en una zona del viewer en compare abre el panel para esa mitad (el slot activo — «Antes»/«Después» — cambia según dónde se clicó respecto al handle del slider); clicar fuera de zona siempre oculta el panel; el cursor cambia a `pointer` sobre zonas clicables en ambas mitades; compare muestra siempre los **2 últimos renders vistos**: izquierda = penúltimo, derecha = último; al activar compare por primera vez el lado izquierdo usa el estado inicial de `baseRender`; `baseRender` también define el fondo del viewer (si está presente, `base.webp` nunca se muestra); al desactivar compare solo el lado derecho permanece visible; en modo compare, cada slot se actualiza de forma independiente sin afectar al otro
- **Modal de información del producto** — botón «ⓘ» en las acciones del viewer abre una modal nativa `<dialog>` con los datos del modelo y variante actualmente visible. En modo zona única muestra nombre de zona, modelo y variante seleccionada; en ambients multi-zona (combined renders) muestra una fila por cada zona; en modo compare muestra dos columnas «Antes» / «Después» con la información de cada lado. Si el modelo tiene el campo `description` en `config.json` (HTML opcional), se muestra sanitizado debajo de los datos de variante. El botón se deshabilita automáticamente cuando no hay render cargado. Al abrir la modal, si el panel de producto está visible se oculta; al cerrar la modal, el panel se restaura al estado previo. La modal se cierra con el botón ×, tecla Escape o click en el backdrop. Cada entrada de producto dentro de la modal incluye dos CTAs:
  - **«Solicita presupuesto»**: botón con `data-modalform_input_name="producto"`, `data-modalform_input_data="Información sobre {modelId} en color {variantName} desde el ambiente {ambientName}"` y `data-modalform_target="lead"`. El enlace apunta a `#modal-lead` (modal-form de WordPress). Al hacer clic cierra la InfoModal.
  - **«Consulta otras medidas disponibles»**: enlace `<a href="/{productFamilyURL}" target="_blank">` cuya URL se configura con el campo `productFamilyURL` en cada modelo de `config.json`. Al hacer clic cierra la InfoModal y abre la URL en nueva pestaña.
- **Descarga PDF** — el botón «Descargar» genera un PDF A4 landscape con: logo de la empresa (`public/assets/logo_PDJIT.svg`), título y nombre del ambiente (cabecera), render a ancho completo y footer con modelo, variante, URL y email de contacto. En modo compare, genera primero un composite de los dos renders separados por la línea del handler en la posición actual del slider, y lo incluye como imagen del PDF. La generación de PDF usa jsPDF con lazy-import (no impacta el bundle inicial). Los campos de branding se configuran con `pdfBranding` en el root de `config.json`
...
| `pdfBranding` | `object` | (Opcional, global) Campos de branding para el PDF descargable |
| `pdfBranding.title` | `string` | Título que aparece en la cabecera del PDF |
| `pdfBranding.siteUrl` | `string` | URL de la web que aparece en el footer del PDF |
| `pdfBranding.contactEmail` | `string` | Email de contacto que aparece en el footer del PDF |
| `model.productFamilyURL` | `string` | (Opcional) Path sin slash inicial de la página de familia de producto. El enlace «Consulta otras medidas disponibles» en la InfoModal construye el href como `"/" + productFamilyURL` (ej. `"productos/adoquines"` → `href="/productos/adoquines"`). Si está ausente o vacío, el enlace no se muestra |
...
**Slots independientes en compare:** `App.jsx` instancia `useRenderLoader` dos veces en paralelo — una para el slot derecho y otra para el slot izquierdo (`leftZoneSelections`). El estado de cada slot persiste al desactivar y reactivar compare dentro del mismo ambient; se resetea al cambiar de ambient.
...
**Rolling base (penúltimo render):** cada vez que el usuario selecciona un nuevo render en modo normal, la selección anterior se guarda automáticamente en `leftZoneSelections`. La capa izquierda (`leftShownUrl`) se pre-renderiza siempre en segundo plano cubierta por el render actual; al activar compare, `selectedUrl` se recorta hacia la derecha revelando la capa izquierda ya pintada — sin flash y sin re-carga. `ambient-base` es siempre la foto original estática (no sigue las selecciones del usuario).
